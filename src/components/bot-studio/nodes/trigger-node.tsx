...
import { TRIGGER_LABELS, getTriggerPreview, type TriggerType } from '@/lib/bot-studio/node-definitions';
  ...
  return (
    <div className="relative group">
      ...
      <div className={`flex flex-col items-center gap-2 ${selected ? 'opacity-100' : ''}`}>
        ...
        <div className={`rounded-xl border-2 bg-white px-4 py-2 shadow-sm text-center min-w-[170px] transition-all ${
          selected ? 'border-gray-900' : 'border-gray-200 hover:border-gray-400'
        }`}>
          <p className="text-[11px] font-medium text-gray-400 uppercase tracking-wide mb-0.5">START — TRIGGER</p>
          <p className="text-[13px] font-bold text-gray-900 leading-snug">{label}</p>
          <p className="text-[10px] text-gray-400 mt-0.5 max-w-[150px] truncate">{getTriggerPreview(cfg)}</p>
        </div>
      </div>

      <div className="absolute right-0 top-[calc(50%-8px)] translate-x-full pl-2">
