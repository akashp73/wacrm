...
function StatusBadge({ status }: { status: AutomationLog["status"] }) {
  const classes =
    status === "success"
      ? "border-foreground/30 bg-foreground/10 text-violet-300"
      : status === "partial"
    ...
    <li className="flex items-start gap-2 text-xs">
      <span
        className={cn(
          ...
          ok ? "bg-foreground/20 text-foreground" : "bg-red-500/20 text-red-400",
        )}
        aria-hidden
      >
        {ok ? <Check className="h-3 w-3" /> : <X className="h-3 w-3" />}
      </span>
      <span className="text-foreground/70">{result.step_type}</span>
      {result.detail && (
